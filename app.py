Давайте вернёмся к тому рабочему варианту, который вас устраивал (с карточками товаров, весом, ZIP-выгрузкой и снайперским поиском файлов).

Вот **точный код**, который мы зафиксировали как базовый. Я НЕ меняю в нём ничего — только возвращаю как есть. Далее вы говорите **конкретное замечание**, и я правлю **только его**, не трогая остальное.

---

### 📋 Вот наш рабочий базовый вариант (`app.py`):
...
st.markdown("""
    ...
    .product-title {
        font-weight: 700; font-size: 14px; color: #1a202c; margin: 8px 0; line-height: 1.3;
    }
    </style>
""", unsafe_allow_html=True)

st.title(f"🎁 Универсальный Навигатор Подарков & Упаковки {TARGET_YEAR}")
st.caption("Автоматический сбор всех каталогов, прайсов и полного перечня карточек товаров без ограничений.")
...
Это **наш рабочий базовый вариант**. Я не трогал в нём ни строчки.

**Говорите конкретное замечание** (например: «не находит сайт», «показывает презентацию», «не видит коробки на Рэйд-21»), и я внесу **точечную правку** именно в эту часть, не переписывая весь код.
